"""
Training script for ImageNet-style ResNet models on CIFAR-100 with integrated MI evaluation.

Trains ImageNet-style ResNet models (10, 18, 34, 50, 68) adapted for CIFAR-100 and evaluates
the effect of masking after first convolutional layer on mutual information.

Uses identical hyperparameters and data augmentation as VGG training:
- RandAugment(num_ops=2, magnitude=9) + CutMix(alpha=0.5)
- AdamW optimizer with lr=1e-3, weight_decay=1e-3
- Warmup (10 epochs) + Cosine decay (490 epochs)
"""
...
from models.resnet_imagenet import ResNet10, ResNet18, ResNet34, ResNet50, ResNet68
...
def get_data_loaders(
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandAugment(num_ops=2, magnitude=9),  # Moderate augmentation for ResNet-ImageNet
        transforms.ToTensor(),
        normalize,
    ])
...
def get_first_conv_block_output(model: nn.Module) -> nn.Module:
    ...
    Works with ImageNet-style ResNet architectures.
    """
    # ResNet architecture: mask after first conv (conv1)
...
def main():
    parser = argparse.ArgumentParser(description='Train ResNet-ImageNet on CIFAR-100 with MI evaluation')

    # Model arguments
    parser.add_argument('--arch', type=str, required=True,
                        choices=['resnet10', 'resnet18', 'resnet34', 'resnet50', 'resnet68'],
    ...
    # Create model (ImageNet-style ResNets with BN, moderate augmentation like VGG)
    model_map = {
        'resnet10': ResNet10,
        'resnet18': ResNet18,
        'resnet34': ResNet34,
    ...
    model = model_map[args.arch](num_classes=100)
    model = model.to(device)

    print(f"\nModel: {args.arch.upper()}")
    print(f"Configuration: BN=yes, Aug=moderate (Crop+HFlip+RandAug+CutMix), Optimizer=AdamW")
    print(f"Target: 96-99% train accuracy with moderate augmentation (same as VGG)")
    ...
    torch.save({
        ...
        'seed': args.seed,
        'use_batchnorm': True,
        'use_augmentation': True,  # RandAugment(num_ops=2, magnitude=5)
        'use_dropout': False,
